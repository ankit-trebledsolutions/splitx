...
const filterMenu = (menuItems, user) => {
  return menuItems
    .filter((item) => {
...
export function SidebarMenu() {
  ...
  const filteredMenu = useMemo(() => {
    ...
    return filterMenu(MENU_SIDEBAR, user);
  }, [user]);
  ...
  const matchPath = useCallback(
    (selectedValue, itemValue) => {
      // Exact match
      if (selectedValue === itemValue) return true;

      // Parent-child match for nested routes
      if (itemValue && selectedValue.startsWith(itemValue + '/')) return true;

      // Special case for general settings - match both parent and children
      if (
        itemValue === '/layout-1/general-settings' &&
        (selectedValue === '/layout-1/general-settings' ||
          selectedValue.startsWith('/layout-1/general-settings/'))
      ) {
        return true;
      }
      ...
      return (
        ...
        (itemValue &&
          itemValue.length > 1 &&
          selectedValue.startsWith(itemValue) &&
          itemValue !== '/layout-1')
      );
    },
    [pathname],
